...
```
### Group Member 1: Gayatri Pai
### Group Member 2: Natalee Morris
### Group Member 3: Adam Duvall
### Group Member 4: Spencer Segal
...
``` The equation is: lm(out$Y ~ out$V1+out$V2). The estimates are not biased because both variables are present. ```
...
``` The equation is: lm(out$Y ~ out$V1). The estimate is biased because only V1 is present; the weight of V2 is shifted onto V1. ```
...
``` The equation is: lm(Y_adj ~ out$V1), where Y_adj <- out$Y - B2*out$V2. 
When rearranging the equation using Y_adj, the equation is the same as in question 1. This means there is no bias because both V1 and V2 are present. ```
...
Y_adj <- out$Y - B2*out$V2
 plotted <- ggplot(out, aes(V1, y = value, color = variable)) + 
  geom_point(aes(y = Y, col = "Y")) + geom_point(aes(y = Y_adj, col = "Y_adj")) +
  geom_smooth(method='lm', aes(y = Y, col = "Y regression")) +
  geom_smooth(method='lm',aes(y = Y_adj, col = "Y_adj regression"))

plot(plotted)

```

``` The new graph has more colors to make it easier to view (as seen through the legend). ```
