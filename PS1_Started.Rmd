...
plot(lm(out$Y ~ out$V1 +out$V2 ))

This regression is not biased because both variables are represented in the regression equation above.
```
...
```
lm(out$Y ~ out$V1)
This regression is biased because it omitts V2 and distributes the wieght onto the V1 coefficient. Thus V1 is acting as an indirect proxy to V2.
...
Y_Adjusted <- out$Y -B2*out$V2
lm(formula = Y_Adjusted ~ out$V1)
on the left y adjusted-b2v2 = b1v1 bc its telling us to regress on V1 so its not biased because we are getting the same expression as before. Becasue y adj includes b2V2 if you rearrange the equation you will get the original equation of Y =b1V1 +B2V2. There is no significant change in the coefficiant of V1 because V2 is present in the equation by being a part of Y_Adjusted.
...
```
To improve this visualization, we changed the colors of the different entities. We made sure that the different regression lines were colored differently and to have it labeled on the legend. We also made Y and Y_Adjusted different colors to make their differences more apparent. They are also included in the legend.
```
...
plotted<-ggplot(out, aes(V1, y = value, color = variable)) + 
  ...
  geom_point(aes(y = Y_Adjusted, col = "Y_Adjusted"))+
  geom_smooth(method='lm', aes(y = Y, col = "reglineY"))+
  geom_smooth(method='lm', aes(y = Y_Adjusted, col = "reglineY_Adjusted"))
plot(plotted)
