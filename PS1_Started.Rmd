...
**Answer:**
```
lm(out$Y ~ out$V1 +out$V2) Our estimates of B1 and B2 are not biased as both variables are represented in the original regression equation. 
```
...
**Answer:**
```
lm(formula = out$Y ~ out$V1) Our estimates are biased as our regression equation does not include our V2 or B2 variable, meaning that B1 variable will hold more weight in it's explanatory predictions than it would in a situation of appropriate weight for both V1 and V2.V1 therefore is acting as a proxy for V2.   
```
...
**Answer:**
```
Y_adj <- out$Y -B2*out$V2  lm(formula = Y_adj ~ out$V1)  As we are able to manipulate the given information to make Yadj equal our original equation from above question one, which includes B1V1 and B2V2, we are able to say that our estimate for B1 is not biased as both variables and their explanatory values of B1 and B2 are appropriately respresented. There is also no significant change between the coefficient for B1 from question 1 (2.988) and this question (2.977), further suggesting a lack of bias.  
```
...
plotted<-ggplot(out, aes(V1, y = value, color = variable)) + 
     ...
     geom_smooth(method='lm', aes(y = Y, col = "regline"))+
     geom_smooth(method='lm', aes(y = Y_adj, col = "regline")) 
plot(plotted)

```
```
The improved graph above forces our Y and Yadj varaibles to take colors that are different than the color of the regression lines, making the slopes of each regression more clear.  Our legend also now includes an indcator for the Y variable which was omitted from the original code.  
```
